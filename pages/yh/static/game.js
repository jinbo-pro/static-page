/**
 * 原代码参考：
 * https://codepen.io/MillerTime/full/XgpNwb
 * 作者：MillerTime
 * 作者主页地址：
 * https://codepen.io/MillerTime/pens/showcase
 */
...
const helpContent = {
  ...
  shellSize: {
    header: '烟花大小',
    body: '烟花的大小。以真实的烟花外壳大小为模型，更大的外壳会产生更大的爆发，伴随着更多的恒星，有时还会产生更复杂的效果。然而，更大的外壳也需要更多的处理能力，并可能导致滞后。'
  },
  quality: {
    header: '画质',
    body: '整体画质。如果动画运行不顺利，请尝试降低质量。高质量会大大增加渲染的火花量，并可能导致延迟。'
  },
  skyLighting: {
    header: '照亮背景',
    body: '当焰火爆炸时照亮背景。如果背景在屏幕上看起来太亮，请尝试将其设置为“暗淡”或“无”。'
  },
  scaleFactor: {
    header: '大小',
    body: '允许缩放所有烟花的大小，实质上是将您移动得更近或更远。对于较大的外壳尺寸，可以方便地将尺寸缩小一点，尤其是在手机或平板电脑上。'
  },
  autoLaunch: {
    header: '自动发射',
    body: '自动发射焰火序列。坐下来欣赏表演，或者禁用以完全控制。'
  },
  finaleMode: {
    header: '狂热模式',
    body: '发射强烈的烟花。可能导致滞后。需要启用“自动发射”。'
  },
  hideControls: {
    header: '隐藏控件',
...
function init() {
  ...
  // shell type
  let options = ''
  shellNames.forEach((opt) => (options += `<option value="${opt}">${shellTypeLabels[opt]}</option>`))
